...
import com.example.prealba.databinding.ActivityAgregarCategoriaLlamadaAdminBinding
import com.google.firebase.auth.FirebaseAuth
import com.google.firebase.database.FirebaseDatabase
import com.google.firebase.storage.FirebaseStorage

class AgregarCategoriaLlamadaAdmin : AppCompatActivity() {
    private lateinit var binding: ActivityAgregarCategoriaLlamadaAdminBinding
    ...
    private var categoriaQSLlAd=""
    private var numeroCategoriaLlaAd = ""

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityAgregarCategoriaLlamadaAdminBinding.inflate(layoutInflater)
    ...
    private fun ValidaDatosCategoriaLlAD() {
        categoriaQSLlAd = binding.EtCategoriaQSLL.text.toString().trim()
        numeroCategoriaLlaAd = binding.EtNumeroQSLL.text.toString().trim()

        if (categoriaQSLlAd.isNullOrEmpty()) {
            Toast.makeText(applicationContext, "Ingrese una categoría", Toast.LENGTH_SHORT).show()
        }
        else if (numeroCategoriaLlaAd.isNullOrEmpty()){
            Toast.makeText(applicationContext, "Ingrese una numero telefonico", Toast.LENGTH_SHORT).show()

        }
        else {
            AgregarLLaAdCategoriaBD()
        }

    }

    private fun AgregarLLaAdCategoriaBD() {
        ...
        val storageRef = FirebaseStorage.getInstance().getReference("imagenes_categoria_Or_Llamadas/$tiempo.jpg")
        ...
        if (imagenUriQS != null) {
            storageRef.putFile(imagenUriQS!!)
                .addOnSuccessListener { taskSnapshot ->
                    ...
                    storageRef.downloadUrl.addOnSuccessListener { uri ->
                        ...
                        hashMap["id"] = "$tiempo"
                        hashMap["categoria"] = categoriaQSLlAd
                        hashMap["numeroTelefonico"] = numeroCategoriaLlaAd
                        ...
                        ref.child("$tiempo")
                            ...
                            .addOnSuccessListener {
                                ...
                                binding.EtCategoriaQSLL.setText("")
                                binding.EtNumeroQSLL.setText("")
                                startActivity(Intent(this@AgregarCategoriaLlamadaAdmin, MainActivity::class.java))
                                finishAffinity()
                            }
            ...
            hashMap["id"] = "$tiempo"
            hashMap["categoria"] = categoriaQSLlAd
            hashMap["numero telefonico"] = numeroCategoriaLlaAd
            hashMap["tiempo"] = tiempo
            hashMap["uid"] = "${firebaseAuth.uid}"
